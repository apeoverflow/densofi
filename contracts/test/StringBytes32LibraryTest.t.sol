...
contract StringBytes32LibraryTest is Test {
    ...
    function testTooLongString() public {
        ...
        string
            memory tooLong = "this-string-is-definitely-too-long-to-fit-in-bytes32-storage.eth";
        bytes memory tooLongBytes = bytes(tooLong);
        console.log("Length of too long string:", tooLongBytes.length, "bytes");
        // This should revert with "String too long for bytes32" but doesn't work with libraries so we will leave it as a comment
        // StringBytes32.stringToBytes32(tooLong);
    }
