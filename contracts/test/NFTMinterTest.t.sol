...
contract NFTMinterTest is Test {
    ...
    function testUriGeneration() public {
        ...
        string memory expectedUri = string(abi.encodePacked("ipfs://", Strings.toString(tokenId)));
    ...
    function testFuzz_MintMultipleDomainsForSameUser(
        ...
        vm.assume(bytes(domainName1).length > 0 && bytes(domainName1).length <= 32);
        vm.assume(bytes(domainName2).length > 0 && bytes(domainName2).length <= 32);
        vm.assume(keccak256(bytes(domainName1)) != keccak256(bytes(domainName2)));
