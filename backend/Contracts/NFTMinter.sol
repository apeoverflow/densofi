...
contract NFTMinter is ERC1155, Ownable {
    ...
    function mintDomainNFT(string memory domainName) public returns (uint256) {
        ...
        require(
            msg.sender == s_domainNameToOwner[domainNameHash],
            "Caller is not the domain owner"
        );

        // Check if domain is available for minting
        require(
            s_isDomainMintable[domainNameHash] == true,
            "Domain not available for minting"
        );

        // Check if domain NFT has not already been minted
        require(
            s_isDomainNFTMinted[domainNameHash] == false,
            "Domain NFT already minted"
        );
     ...
     */
    function setIsDomainMintable(
        string memory domainName,
        bool availableForNFTMinting
    ) public onlyOwner {
        bytes32 domainNameHash = domainName.stringToBytes32();
        s_isDomainMintable[domainNameHash] = availableForNFTMinting;
        emit DomainMintableStatusSet(
            domainNameHash,
            availableForNFTMinting,
            domainName
        );
    }
